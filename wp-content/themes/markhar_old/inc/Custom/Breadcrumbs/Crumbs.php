...
namespace THKKZ\Custom\Breadcrumbs;
...
class Crumbs
{
	...
	public function trail() {
		...
		if ( 0 < $item_count ) {
			...
			foreach ( $this->items as $item ) {
				...
				$item = ! empty( $matches )
					? preg_replace( '/(<a.*?)([\'"])>/i', '$1$2 class="trail-link" itemprop=$2item$2>', $item )
					: sprintf( '<span class="trail-link" itemprop="item">%s</span>', $item );
	...
	protected function set_labels() {
		...
		$defaults = array(
			'browse'              => esc_html__( 'Навигация:',                               '__theme' ),
			'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', '__theme' ),
			'home'                => esc_html__( 'Главная',                                  '__theme' ),
			'error_404'           => esc_html__( 'Страница 404',                         '__theme' ),
			'archives'            => esc_html__( 'Архив',                              '__theme' ),
			// Translators: %s is the search query.
			'search'              => esc_html__( 'Результат поиска: %s',                '__theme' ),
			// Translators: %s is the page number.
			'paged'               => esc_html__( 'Страница %s',                               '__theme' ),
			// Translators: %s is the page number.
			'paged_comments'      => esc_html__( 'Комментарии %s',                       '__theme' ),
			// Translators: Minute archive title. %s is the minute time format.
			'archive_minute'      => esc_html__( 'Минута %s',                             '__theme' ),
			// Translators: Weekly archive title. %s is the week date format.
			'archive_week'        => esc_html__( 'Неделя %s',                               '__theme' ),
